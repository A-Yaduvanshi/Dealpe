...
for ($i = 1; $i <= $quantity; $i++) {
	...
	while ($rowCheck = mysqli_fetch_array($query)) {
		...
		$sql_update = "UPDATE `membership_card` set `assign`='1',`admin_id`= '" . $_SESSION['sess_user'] . "',`assign_date`=current_timestamp(),`assign_name`='" . $franchise_name . "',`asign_count`='1' WHERE `membership_card` = '" . $membership_card . "' AND `assign`!=1 AND `asign_count`!=1";
		// echo $sql_update . "<br>";
