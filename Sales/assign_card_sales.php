...
$six_months = "SELECT * FROM `membership_card` WHERE `sales_select`='1' and `S_id`='$id' AND`sale_count`='1'";
$runsix = mysqli_query($conn, $six_months);
$rowsix = mysqli_num_rows($runsix);
// $unsoldCard = $rowTHree + $rowsix
$sql_2 = "SELECT * FROM `membership_card` WHERE `sales_select`='1' and `S_id`='$id' AND`sale_count`='1'";
...
<body class="bg-dark m-5">
    <div class="container-fluid container-md container-sm border border-primary bg-white">
        <header style="font-weight: bold; color:brown;text-align:center;text-decoration:underline;font-size:30px;">Assign Cards To Customer</header>
        ...
        <div class="container-fluid container-md container-sm  bg-white mt-5">
            ...
            <div class="row mx-2 p-4 border">
                <h5 class="m-3">Sales Executive name</h5 class="m-3">
                <div class="col-3  ">
                    <!-- <select id="franchise" onchange="selectchange()"> -->
                    <input list="list" name="name" id="customer" onchange="selectchange()">
                    <datalist id="list">
                        <option value="" selected="selected">Select Employee Name</option>
                        <?php
                        include "../api/connection.php";
                        $sql = "SELECT * FROM `customer` where `sale_id`='".$id."'";
            ...
            <form action="assignCard.php" method="POST" enctype="multipart/form-data">
                <div class="container-fluid container-md container-sm ">
                    ...
                    <div class="row " style="align-items: center; align-self: center;">
                        ...
                        <div class="col-3 m-4 ">
                            <label>Enter Sales Executive Id No.</label>
                            <input type="text" placeholder="Enter Franchise ID no." name="id" required>
                        </div>
                        <div class="col-2 m-4">
                            <label>Sales Executive Name</label>
                            <input type="text" placeholder="Enter your name" name="sale_name" required>
                        </div>
                        <!-- <div class="col-3 m-4">
    ...
    <!-- <script>
        $(document).ready(function() {
            $(".mul-select").select2({
                placeholder: "select Membership Cards",
                tags: true,
            });
        })
    </script> -->
